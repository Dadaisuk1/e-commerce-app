...
export default function CartPage() {
  ...
  const [showLoginAlert, setShowLoginAlert] = useState(false); // State to control dialog visibility
  ...
  const handleClearCart = () => {
    // Could replace this confirm with an AlertDialog too
  ...
  const handleProceedToCheckout = () => {
    if (currentUser) {
      ...
      // If not logged in, show the Alert Dialog instead of alert()
      setShowLoginAlert(true);
    }
  };

  // Action for the Alert Dialog's "Login" button
  const handleGoToLogin = () => {
    router.push("/login");
    // No need to setShowLoginAlert(false) as navigation will unmount this state
  };
  ...
  return (
    <>
      ...
      <AlertDialog open={showLoginAlert} onOpenChange={setShowLoginAlert}>
        {/* No Trigger needed as we open it via state */}
